...
class _ActionHeadConfig:
    """
    Lightweight config for action head testing.
    Holds action-head-specific shape constants that InferenceConfig does not include.
    """
    def __init__(
        self,
        batch_size: int = 1,
...
def _create_action_head_config(
    ...
    Defaults are small for fast CPU unit testing.

    Examples:
        # SmolVLA
        cfg = _create_action_head_config(action_chunk_size=50, action_dim=6,
                                          num_conditioning_tokens=512,
                                          conditioning_hidden_size=960)
        # GR00T N1.7
        cfg = _create_action_head_config(action_chunk_size=40, action_dim=132,
...
def _make_denoiser_inputs(cfg: _ActionHeadConfig, seed: int = 42, zeros: bool = False):
    """Generate inputs matching NeuronDenoisingWrapper.forward() signature."""
    if zeros:
        return (
            torch.zeros(cfg.batch_size, cfg.action_chunk_size, cfg.action_dim, dtype=cfg.dtype),
            torch.zeros(cfg.batch_size, cfg.num_conditioning_tokens, cfg.conditioning_hidden_size, dtype=cfg.dtype),
            torch.zeros(cfg.batch_size, cfg.timestep_embed_dim, dtype=cfg.dtype),
            torch.zeros(cfg.batch_size, 1, cfg.action_chunk_size, cfg.num_conditioning_tokens, dtype=torch.int32),
        )
    torch.manual_seed(seed)
    return (
        torch.randn(cfg.batch_size, cfg.action_chunk_size, cfg.action_dim, dtype=cfg.dtype),
        torch.randn(cfg.batch_size, cfg.num_conditioning_tokens, cfg.conditioning_hidden_size, dtype=cfg.dtype),
        torch.randn(cfg.batch_size, cfg.timestep_embed_dim, dtype=cfg.dtype),
        torch.ones(cfg.batch_size, 1, cfg.action_chunk_size, cfg.num_conditioning_tokens, dtype=torch.int32),
    )


def test_action_head_correctness(
    neuron_wrapper_class,
    pytorch_wrapper_class,
    weight_mapping,
    action_head_config=None,
    inference_config=None,
    neuron_init_kwargs=None,
    pytorch_init_kwargs=None,
    ...
    PyTorch reference. Tests one denoising step — use test_denoising_loop_correctness
    for full loop testing.
    """
    ...
    if verbose:
        ...
        print(f"  action_chunk_size={cfg.action_chunk_size}, action_dim={cfg.action_dim}")
        print(f"{'=' * 80}\n")
...
def test_denoising_loop_correctness(
    ...
    Args:
        neuron_generate_actions_fn:  (conditioning_tokens, num_steps) -> Tensor
        pytorch_generate_actions_fn: (conditioning_tokens, num_steps) -> Tensor
        action_head_config:          _ActionHeadConfig
        atol:                        1e-2 default accounts for BF16 accumulation
    """
    cfg = action_head_config or _create_action_head_config()
    ...
    if verbose:
        ...
        print(f"  num_steps={cfg.num_denoising_steps}, action_chunk={cfg.action_chunk_size}")
        print(f"{'=' * 80}\n")
    ...
    with torch.no_grad():
        neuron_out = neuron_generate_actions_fn(conditioning, cfg.num_denoising_steps)
        pytorch_out = pytorch_generate_actions_fn(conditioning, cfg.num_denoising_steps)

    n_fp32 = neuron_out.float()
    p_fp32 = pytorch_out.float()

    assert not torch.isnan(n_fp32).any(), "NaN in Neuron output"
    assert not torch.isinf(n_fp32).any(), "Inf in Neuron output"
    assert n_fp32.abs().max().item() < 10.0, (
        f"Output outside plausible action range: max={n_fp32.abs().max():.4f}"
    )

    max_diff = (n_fp32 - p_fp32).abs().max().item()
    mean_diff = (n_fp32 - p_fp32).abs().mean().item()

    if verbose:
        print(f"  max_diff={max_diff:.6f}  mean_diff={mean_diff:.6f}  atol={atol}")

    assert max_diff <= atol, (
        f"Final action chunk mismatch: max_diff={max_diff:.6f} > atol={atol}"
    )
...
def test_action_head_cpu_interface(
    ...
    """
    Smoke test that NeuronActionHeadBase subclass instantiates correctly on CPU
    and exposes the required interface. Does NOT test numerical correctness.
    """
    cfg = action_head_config or _create_action_head_config()
    ...
    try:
        ...
        raise AssertionError(f"ImportError — Neuron CPU fallback broken: {e}")
    ...
    try:
        ...
        if verbose:
            print(f"  ✓ get_conditioning_contract() OK: "
                  f"tokens={contract.num_conditioning_tokens}, "
                  f"hidden={contract.conditioning_hidden_size}")
    except NotImplementedError:
        raise AssertionError(f"get_conditioning_contract() not implemented")

    try:
        dummy = torch.zeros(cfg.batch_size, cfg.num_conditioning_tokens,
                            cfg.conditioning_hidden_size, dtype=cfg.dtype)
        head.generate_actions(dummy, num_steps=1)
        raise AssertionError("generate_actions() should raise before compile_denoiser()")
    except AssertionError as e:
        if "should raise" in str(e):
            raise
        if verbose:
    ...
    try:
        ...
        assert isinstance(timesteps, list), f"Must return list, got {type(timesteps)}"
        assert len(timesteps) == 3
        assert all(isinstance(t, float) for t in timesteps), \
            f"Must return Python floats: {[type(t) for t in timesteps]}"
        if verbose:
            print(f"  ✓ _get_timestep_sequence() returns Python floats: {timesteps}")
    except NotImplementedError:
        raise AssertionError("_get_timestep_sequence() not implemented")

    if verbose:
        print(f"\n{'=' * 80}")
        print(f"✓ CPU INTERFACE SMOKE TEST PASSED")
        print(f"{'=' * 80}\n")
