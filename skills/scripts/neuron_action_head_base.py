...
class NeuronDenoisingWrapper(ModelWrapper):
    ...
    def input_generator(self) -> List[Tuple[torch.Tensor, ...]]:
        ...
        NOTE: This default implementation assumes cfg exposes action_chunk_size,
        action_dim, num_conditioning_tokens, conditioning_hidden_size, and
        timestep_embed_dim as top-level attributes. These come from the
        model-specific InferenceConfig subclass, not from the base NeuronConfig.
        Ensure your InferenceConfig subclass sets all five before calling this.
        """
        cfg = self.config
...
class NeuronActionHeadBase(NeuronApplicationBase):
    ...
    def _get_timestep_sequence(self, num_steps: int) -> List[float]:
        ...
        For flow matching (GR00T, SmolVLA, π0):
            return [1.0 - i / num_steps for i in range(num_steps)]

        For DDPM-style (if encountered — rare in current VLAs):
            return list of beta schedule values in reverse order.
            Verify against the model's noise scheduler source before implementing.
        """
        raise NotImplementedError
    ...
    def compile_denoiser(self, save_path: str) -> None:
        ...
        Note: On CPU (no Neuron hardware), compile() is a no-op — the wrapper
        is built but not compiled. This allows unit testing of the interface
        without hardware. compile() only runs when _NEURON_AVAILABLE is True.
        """
        self.denoising_wrapper = self._build_denoising_wrapper()
        self.attention_mask = self._build_attention_mask()
        # Guard: compile() does not exist on nn.Module (CPU fallback path).
        # On Neuron hardware this delegates to NxDI compile infrastructure.
        # Subclass may override to pass custom compiler args.
        if _NEURON_AVAILABLE and hasattr(self.denoising_wrapper, 'compile'):
